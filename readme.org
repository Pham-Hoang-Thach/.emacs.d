...
#+begin_src elisp
  (defun butler/extract-correct-answers (file-path)
    "Extract correct answers with question numbers and save them to a CSV file."
    (interactive "FEnter file path to save answers (as CSV): ") ; Prompt user for file path
    (let ((answers '())
          (question-number 1)) ; Start question numbering from 1
      (save-excursion
        (goto-char (point-min)) ; Start at the beginning of the buffer
        (while (re-search-forward "^:Correct: \\(.*\\)$" nil t)
          (let ((answer (match-string 1))) ; Capture the answer
            (push (format "%d,%s" question-number answer) answers) ; Format as CSV row
            (setq question-number (1+ question-number))))) ; Increment question number
      (setq answers (reverse answers)) ; Reverse the list for correct order
      (with-temp-file file-path ; Write answers to the specified file
        (insert "Question,Answer\n") ; Add a CSV header
        (dolist (answer answers)
          (insert answer "\n"))) ; Write each answer in a new line
      (message "Answers saved to %s" file-path))) ; Notify the user
#+end_src
