...
#+begin_src elisp
  ...
  (setq pdf-annot-save-annotations nil)
  (setq pdf-view-use-scaling nil)
  (add-hook 'pdf-view-mode-hook
            (lambda () (auto-save-mode -1)))
  ;; Displaying PDFs Inline (like images)
  (add-to-list 'org-file-apps '("\\.pdf\\'" . org-pdfview-open))
  (add-to-list 'image-file-name-extensions "pdf")
  ;; Let Org use ImageMagick to rasterize PDFs
  (setq org-image-actual-width nil) ;; auto-scale
...
#+begin_src elisp
  ...
  (setq org-roam-node-display-template
        (concat "${type:12} ${title:120} " (propertize "${tags:0}" 'face 'org-tag)))
  ;; Template
  (setq org-roam-capture-templates
        '(("a" "agendas" plain "%?"
           :if-new
             (file+head "agendas/%<%Y%m%d%H%M%S>-${slug}.org"
                        ":PROPERTIES:\n:ROAM_REFS: ${ref}\n:END:\n#+filetags:\n#+title: ${title}\n")
             :unnarrowed t)
          ("b" "books" plain "%?"
           :if-new
          ...
          ("d" "discussion" plain "%?"
           :if-new
             (file+head "discussion/%<%Y%m%d%H%M%S>-${slug}.org"
                        ":PROPERTIES:\n:ROAM_REFS: ${ref}\n:END:\n#+filetags:\n#+title: ${title}\n")
             :unnarrowed t)
          ("e" "emacs" plain "%?"
           :if-new
           (file+head "emacs/%<%Y%m%d%H%M%S>-${slug}.org"
                      ":PROPERTIES:\n:ROAM_REFS: ${ref}\n:END:\n#+filetags:\n#+title: ${title}\n")
           :unnarrowed t)
          ("h" "home" plain "%?"
           :if-new
          ...
          ("j" "journals" plain
           ...
           :target
           (file+head "%(expand-file-name (concat (or citar-org-roam-subdir \"\") \"journals/\" (format-time-string \"%Y%m%d%H%M%S\") \"-\" \"${citar-citekey}.org\") org-roam-directory)"
            ":PROPERTIES:\n:ROAM_REFS: ${ref}\n:END:\n#+filetags:\n#+title: ${citar-title}\n")
           :unarrowed t)
          ("l" "labs" plain "%?"
           :if-new
          ...
          ("n" "news" plain "%?"
           :if-new
           (file+head "news/%<%Y%m%d%H%M%S>-${slug}.org"
                      ":PROPERTIES:\n:ROAM_REFS: ${ref}\n:END:\n#+filetags:\n#+title: ${title}\n")
           :unnarrowed t)
          ("o" "otexts" plain "%?"
           :if-new
           (file+head "otexts/%<%Y%m%d%H%M%S>-${slug}.org"
                      ":PROPERTIES:\n:ROAM_REFS: ${ref}\n:END:\n#+filetags:\n#+title: ${title}\n")
           :unnarrowed t)
          ("p" "projects" plain "%?"
           :if-new
          ...
          ("s" "slides" plain "%?"
           :if-new
           (file+head "slides/%<%Y%m%d%H%M%S>-${slug}.org"
                      ":PROPERTIES:\n:ROAM_REFS: ${ref}\n:END:\n#+filetags:\n#+title: ${title}\n")
           :unnarrowed t)
          ("t" "tutorials" plain "%?"
           :if-new
           (file+head "tutorials/%<%Y%m%d%H%M%S>-${slug}.org"
                      ":PROPERTIES:\n:ROAM_REFS: ${ref}\n:END:\n#+filetags:\n#+title: ${title}\n")
           :unnarrowed t)
          ("u" "university" plain "%?"
           :if-new
           (file+head "university/%<%Y%m%d%H%M%S>-${slug}.org"
                      ":PROPERTIES:\n:ROAM_REFS: ${ref}\n:END:\n#+filetags:\n#+title: ${title}\n")
           :unnarrowed t)
          ("v" "videos" plain "%?"
           :if-new
          ...
          ("w" "workshops" plain "%?"
           :if-new
           (file+head "workshops/%<%Y%m%d%H%M%S>-${slug}.org"
                      ":PROPERTIES:\n:ROAM_REFS: ${ref}\n:END:\n#+filetags:\n#+title: ${title}\n")
           :unnarrowed t)))
#+end_src
...
#+begin_src elisp
      ...
      (use-package citar
        ...
        (citar-bibliography  '("/mnt/tsys/emacs/pkm/BK-wsl.bib"
                               ...
                               "/mnt/tsys/emacs/pkm/OCW-wsl.bib"))
      ...
      (setq citar-templates
            '((main . "${author editor:30%sn}   ${year issued date:4}   ${title:80}")
              (suffix . "")
...
#+begin_src elisp
  ...
  (setq org-capture-templates
        `(("i" "Inbox" entry
         ...
         ("r" "Research" entry
          (file "resdb.org")
          (file "~/.emacs.d/capture/resdb.org")
          :empty-lines-before 1)))
  ...
  (setq org-log-done 'time)      ;; timestamp when DONE
...
#+begin_src elisp
  ...
  ;; Define button type for Org-roam nodes
  ...
  ;; Buffer-local vars to remember filters
  (defvar-local butler/org-roam-dashboard-tags nil)
  (defvar-local butler/org-roam-dashboard-types nil)

  (defun butler/insert-refresh-button (label refresh-fn)
    "Insert a REFRESH button with LABEL that calls REFRESH-FN."
    (insert-text-button label
                        'follow-link t
                        'help-echo "Click to refresh the dashboard"
                        'face `(:background ,(doom-color 'bg)
                                :foreground ,(doom-color 'yellow)
                                :box t)
                        'refresh-fn refresh-fn
                        'action (lambda (btn)
                                  (funcall (button-get btn 'refresh-fn))))
    (insert "\n\n"))

  ;;; Renderer
  (defun butler/org-roam-dashboard-render (tags types)
    "Render Org-roam dashboard with TAGS and TYPES filters."
    (let* ((all-nodes (org-roam-node-list))
      ...
      (with-current-buffer (get-buffer-create "*Org-roam Dashboard*")
        (erase-buffer)
        ;; Save filters
        (setq butler/org-roam-dashboard-tags tags
              butler/org-roam-dashboard-types types)
        ;; REFRESH button
        (butler/insert-refresh-button "[ REFRESH ]"
                                      (lambda ()
                                        (butler/org-roam-dashboard-render
                                         butler/org-roam-dashboard-tags
                                         butler/org-roam-dashboard-types)))
        ;; Header
            ...
            (let* ((title (org-roam-node-title node))
                   ...
                   (tags-shown (if (seq-empty-p tags)
                                 ...
                                 (seq-filter (lambda (tag) (member tag tags-in-node))
                                             tags)))
                   (tags-full (if (seq-empty-p tags-shown) "-" (string-join tags-shown ", ")))
  ...
  (defun butler/org-roam-dashboard ()
    "Prompt for filters and show Org-roam dashboard."
    (interactive)
    (let* ((all-nodes (org-roam-node-list))
           ...
           (types (completing-read-multiple "Filter by types (comma separated, RET for all): "
                                            all-types nil t)))
      (butler/org-roam-dashboard-render tags types)))
#+end_src

** Typewriter
#+begin_src elisp
  (defvar typewriter-delay 0.1
    "Default delay in seconds between characters when simulating typing.")

  (defun butler/typewriter (file &optional delay)
    "Simulate typing FILE into current buffer with DELAY seconds between chars.
  Skips the initial Org property drawer (:PROPERTIES: ... :END:) and the #+filetags line."
    (interactive "fSource file: \nP")
    (let* ((delay (or delay typewriter-delay))
           (content (with-temp-buffer
                      (insert-file-contents file)
                      (buffer-string))))
      ;; Remove only the first property drawer if present
      (when (string-match "\\`.*?:PROPERTIES:[\n\r]\\(.\\|\n\\)*?:END:[\n\r]*" content)
        (setq content (replace-match "" t t content)))
      ;; Remove the first #+filetags line if present
      (when (string-match "^#\\+filetags:.*\n" content)
        (setq content (replace-match "" t t content)))
      ;; Type character by character
      (dolist (char (string-to-list content))
        (insert (string char))
        (sit-for delay)
        (recenter))))
#+end_src
